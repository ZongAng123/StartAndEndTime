...
@interface ZDatePicker ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
#pragma mark - https://blog.csdn.net/u013243469/article/details/42262215
...
- (void)refreshData {
    ...
    if (self.mode == ZDatePickerModeyyyyMMdd) {
...
- (void)initDatePickerView {
    ...
    self.frame = CGRectMake(0, 0, ZAScreenWidth, 260);
}
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    if (self.mode == ZDatePickerModeyyyyMM) {
        return 2;
    } else {
        return 3;
    }
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView
    ...
    if (component == 0) {
        ...
        if (self.isShowToday &&
            ...
            self.defaultYear == self.thisYear &&
            self.defaultMonth == self.thisMonth &&
            (self.mode == ZDatePickerModeyyyyMMdd && self.defaultDay == self.thisDay)) {
            text = @"至今";
        }
        return text;
    } else if (component == 1){
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.defaultYear < self.minimumYear) {
        ...
        if (self.defaultMonth < self.minimumMonth) {
            ...
            if (self.defaultDay < self.minimumDay && self.mode == ZDatePickerModeyyyyMMdd) {
    ...
    if (self.defaultYear > self.maximumYear) {
        ...
        if (self.defaultMonth > self.maximumMonth) {
            ...
            if (self.defaultDay > self.maximumDay  && self.mode == ZDatePickerModeyyyyMMdd) {
...
- (UIView *)line {
    if (!_line) {
        _line = [[UIView alloc] initWithFrame:CGRectMake(0, 39.5, ZAScreenWidth, 0.5)];
...
- (UIToolbar *)toolBar {
    if (!_toolBar) {
        _toolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, ZAScreenWidth, 44)];
...
- (ZPickerView *)pickerView {
    if (!_pickerView) {
        _pickerView = [[ZPickerView alloc] initWithFrame:CGRectMake(0, 44, ZAScreenWidth, 216)];
